...
if echo "$URL" | grep -q "reddit.com"; then
    ...
    if tty -s; then
        ...
        kitty --class "floating_kitty" -e bash -c "echo 'Starting Reddit download...'; yt-dlp -o '$HOME/Videos/%(title)s.%(ext)s' '$URL'; echo 'Download complete! Press Enter to close...'; read"
    fi
